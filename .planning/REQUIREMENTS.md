...
- [x] **CACHE-05**: Application behaviour is identical to pre-cache — no functional regressions in auth flows, data access, or export functionality; absent Upstash env vars trigger fail-open (cache wrappers fall through to Supabase and log the error)
